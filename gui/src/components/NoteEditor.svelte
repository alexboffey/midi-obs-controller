<script lang="ts">
  ...
  function setActionType(type: ActionConfig['action']) {
    if (type === 'loop') {
      store.update(noteStr, { action: 'loop', prefix: 'LOOP_A_', style: 'cycle', bpm: 120, steps: 4 })
    } else if (type === 'static') {
...
{#if entry}
  <div class="space-y-6">
    ...
    <!-- Action type picker -->
    <div>
      <label class="block text-xs font-semibold text-gray-400 uppercase tracking-wider mb-2">Action type</label>
      <div class="flex gap-2 flex-wrap">
        {#each (['loop', 'static', 'sequence'] as const) as type}
          <button
